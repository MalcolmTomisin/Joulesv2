...
public class ArtistAlbumLoader {
        ...
        if (cursor != null){
            if (cursor.moveToFirst())
                do {
                    ...
                    Album album = new Album(cursor.getLong(0), cursor.getString(1),
                            cursor.getString(2),artistID, cursor.getInt(3),
                            cursor.getInt(4));
                    albumList.add(album);
    ...
    public static Cursor makeAlbumForArtistCursor(Context context, long artistID){
        ...
        Cursor cursor = context.getContentResolver().query(MediaStore.Audio.Artists.Albums.getContentUri("external",artistID),
                new String[]{"_id", "album", "artist", "numsongs", "minyear"}, null, null,
